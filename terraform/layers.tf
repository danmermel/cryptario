...
resource "aws_s3_object" "apiLayerZipObject" {
  bucket = aws_s3_bucket.cryptarioLambdaCode.id
...
resource "aws_lambda_layer_version" "cryptarioLambdaLayer" {
  layer_name          = "cryptarioLambdaLayer-${terraform.workspace}"
  s3_bucket           = aws_s3_bucket.cryptarioLambdaCode.id
